...
<html lang="en">
   ...
   <body class="fixed-bottom-padding" onload="loaderStop()">
         ...
         <div class="bg-color-head">
            <div class="container menu-bar d-flex align-items-center">
               <ul class="list-unstyled form-inline mb-0">
                  ...
                  <li class="nav-item active">
                     <a class="nav-link text-white pl-0" href="{{URL::to('/grocery')}}">Grocery </a>
                  </li>
                  <li class="nav-item dropdown">
                     ...
                     <div class="dropdown-menu" aria-labelledby="navbarDropdown">
                        @foreach($category_all as $category)
                           <a class="dropdown-item" href="{{ URL::to('/restaurant_product_category/'.$category->id) }}">
                           {{$category->category}}
                           </a>
                        @endforeach
                     </div>
                  ...
                  <!-- <li class="nav-item active">
                   ...
                   @if(session()->has('client_id'))
                      <li class="nav-item active">
                         <a class="nav-link text-white pl-0" href="{{URL::to('/res_MyOrder')}}">My Order </a>
                      </li>
                   @endif
                  <!-- <li class="nav-item dropdown">
                     <a class="nav-link text-white dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
         ...
         <section class="footer-main border-top pt-4 pb-4">
            ...
            <div class="col-lg-2 col-md-2 mb-30" >
                <div class="footer-widget pament-widget">
                    ...
                    <div class="row">
                        <div class="col-md-6 col-3"><a href="#!"><img src="{{asset('robi.png')}}" alt="" height="50"></a></div>
                        <div class="col-md-6 col-3"><a href="#!"><img src="{{asset('bKash-logo.png')}}" alt="" height="50"></a></div>
                    </div>
                    <br>
                </div>
            </div>
      ...
      <?php
          $cart_data=DB::table('restaurant_temp_cart')->where('client_id',session('client_id'))->get();
          $cart_item=count($cart_data);
          $total_price=0;
          ?>
          @include('restaurant/layouts/cart_sidebar')
          @include('grocery/layouts/mobile_sidenav')
      <a href="#" data-toggle="modal" data-target="#cart" class="mr-2 text-dark bg-light  p-2 icofont-size border shadow-sm floating_cart" style="z-index:999;text-align:center;line-height: 0.8rem;background-color: #7ed957 !important;box-shadow: 0px 0px 3px #00000069!important;font-size: 25px;color: #333 !important;top: 450px;position: fixed;right: 0; width:57px;border: none !important;">
      ...
      <script>
          ...
          jQuery(document).ready(function(){
              ...
              if(window.location.href=="{{url('/resCheckout?')}}"){
              ...
              var paymentConfig={
                  createCheckoutURL: '{{url('/resCreatepayment')}}',
                  executeCheckoutURL: '{{url('/resExecutepayment')}}'
              };


              var paymentRequest;
              paymentRequest = { amount:jQuery("#bKash_button").attr('name'),intent:'sale',invoice:'{{'KTW-'.date('md').'-'.strtoupper(Str::random(4))}}'};
              ...
              bKash.init({
                  ...
                  executeRequestOnAuthorization: function(){
                      ...
                      $.ajax({
                          url: paymentConfig.executeCheckoutURL+"?paymentID="+paymentID + "&delivery_note=" + jQuery(".delivery_note").val(),
                          type: 'GET',
                          contentType:'application/json',
                          success: function(data){
                              ...
                              if(data && data.paymentID != null){
                                  ...
                                  window.location.href = '{{url('/res_MyOrder')}}';
                              }
                              else {
              ...
              jQuery(".add_to_cart").click(function(e){
                  ...
                  $.ajax({
                      url: '{{url('/restaurant/product/add_to_cart')}}',
                      type: 'post',
                      data: {
                      ...
                      success: function(data) {
                          ...
                          if(data.message=='Product found!'){
                              ...
                              const markup = "<tr class='"+data.product_id1+"row' style='width: 100%;border-bottom: 2px solid #9e9e9e45;'>" +
                                  ...
                                  "<input type='button' value='-' name='"+data.price+"' class='qtyminus btn btn-success btn-sm' field='" + data.product_id1 + "quantity' />" +
                              ...
                              window.location = "{{ URL::to('/restaurant') }}";
                          }

                          jQuery(".proceed_to_checkout").remove();
                          const checkout_btn = "<button style='border-radius: unset !important;' formaction='{{URL::to('/resCheckout')}}' class='btn btn-success btn-lg rounded btn-block proceed_to_checkout' ><i class='far fa-credit-card'></i> Proceed to Checkout</button>";
              ...
              jQuery(".add_to_cart_details").click(function(e){
                  ...
                  $.ajax({
                      url: '{{url('/restaurant/product/add_to_cart_details')}}',
                      type: 'post',
                      data: {
                          product_id: product_id,
                      ...
                      success: function(data) {
                          ...
                          if(data.message=='Product found!'){
                              ...
                              const markup = "<tr class='"+data.product_id1+"row' style='width: 100%;border-bottom: 2px solid #9e9e9e45;'>" +
                                  ...
                                  "<span class='ml-auto' href='#'>" +
                                  "<form id='' class='cart-items-number d-flex' >" +
                                  "<input type='button' value='-' name='"+data.price1+"' class='qtyminus btn btn-success btn-sm' field='" + data.product_id1 + "quantity' />" +
                                  "<input type='text' name='" + data.product_id1 + "quantity' value='" + data.quantity + "' class='qty form-control " + data.product_id1 + "' readonly/>" +
              ...
              jQuery('body').on('click', '.qtyplus', function(e) {
                  ...
                  $.ajax({
                      url: '{{url('/restaurant/product/add_to_cart_plus')}}',
                      type: 'post',
                      data: {
                          product_id: product_id
                      },
              ...
              jQuery('body').on('click', '.qtyminus', function(e) {
                  ...
                  if(currentVal>1){
                      ...
                      $.ajax({
                          url: '{{url('/restaurant/product/add_to_cart_minus')}}',
                          type: 'post',
                          data: {
                              product_id: product_id
                          },
              ...
              jQuery('body').on('click', '.cart_remove', function(e) {
                  ...
                  if(total_price_int<=0){
                      jQuery(".proceed_to_checkout").remove();
                      const checkout_btn = "<button style='border-radius: unset !important;' formaction='#' class='btn btn-success btn-lg rounded btn-block emptyCart proceed_to_checkout' ><i class='far fa-credit-card' onclick=''></i> Proceed to Checkout</button>";
                      jQuery(".checkout_btn_holder").append(checkout_btn);
                  }

                  $.ajax({
                      url: '{{url('/restaurant/product/remove_cart')}}',
                      type: 'post',
                      data: {
                          product_id: product_id
                      },
                    ...
                    success: function(data) {
                        ...
                        if (data.message=='Found') {
                            for(var i=0; i<data.product.length; i++){
                                ...
                                /*var result = doesFileExist("/public"+data.product[i].image);
                                if (result == true){*/
                                    var tr_str = "<tr id='search_row' style='width: 100%;border: 2px solid #f0f2f542;background-color: #607d8b87;'>"+
                                        "<td style='width: 30%; text-align: center;'>"+
                                        "<a href='/restaurant_product_details/"+btoa(data.product[i].id)+"'>"+
                                        "<img height='50px' src='https://manage.naturexbd.com"+data.product[i].image +"' alt='No Img'>"+
                                        "</a>"+
                                        "</td>"+
                                        "<td style='width: 70%; text-align: left;'>"+
                                        "<a style='color:#fff;text-shadow: 0px 0px 2px #000;font-weight: 600;' href='/restaurant_product_details/"+btoa(data.product[i].id)+"'>"+
                                        data.product[i].name+
                                        "</a>"+
                                        "</td>"+
                                        "</tr>";
