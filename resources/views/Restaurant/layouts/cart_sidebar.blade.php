                ...
                <nav class="schedule w-100">
                    <div class="nav nav-tabs" id="nav-tab" role="tablist">
                        ...
                        <a class="nav-link col-8 p-0 d-flex align-items-center justify-content-center" data-dismiss="modal" aria-label="Close">
                            <h1 class="m-0 h4 text-dark">Restaurant Cart</h1>
                        </a>
                        <a class="nav-link col-2 p-0 d-flex align-items-center justify-content-center" data-dismiss="modal" aria-label="Close">
                                ...
                                <td style='width: 30%; text-align: center;'>
                                    <span class='ml-auto' href='#'>
                                        <form id='' class='cart-items-number d-flex' >
                                            <input type='button' value='-' name='{{$cart->unit_price}}' class='qtyminus btn btn-success btn-sm' field='{{$cart->product_id.'quantity'}}' />
            ...
            <div class="modal-footer p-0 border-0">
                <form method="get" class="col-12 m-0 p-0">
                    ...
                    @if(session()->has('client_id'))
                        @if($total_price<=0)
                            <button style="border-radius: unset !important;" formaction="#" class="btn btn-success btn-lg rounded btn-block emptyCart proceed_to_checkout" ><i class="far fa-credit-card" onclick=""></i> Proceed to Checkout</button>
                        @else
                            <button style="border-radius: unset !important;" formaction="{{URL::to('/resCheckout')}}" class="btn btn-success btn-lg rounded btn-block proceed_to_checkout" ><i class="far fa-credit-card"></i> Proceed to Checkout</button>
                        @endif
                    @else
                        <button style="border-radius: unset !important;" formaction="#" class="btn btn-success btn-lg rounded btn-block loginAlert proceed_to_checkout" ><i class="far fa-credit-card" onclick=""></i> Proceed to Checkout</button>
                    @endif
                    </div>
                </form>
