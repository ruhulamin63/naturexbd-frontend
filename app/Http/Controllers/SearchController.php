...
class SearchController extends Controller
{
    public function search_keyword(Request $request)
    {
        ...
        $search_data=DB::table('grocery_products')->where('cityID',$cityID)->where('status',"Active")->where(function($q) {$q->where('product_type',1)->orwhere('product_type',2);})->where('product_name','like','%'.$keyword.'%')->take(10)->get();
