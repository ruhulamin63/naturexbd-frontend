        ...
        if ($resultdatax && $resultdatax->paymentID != null && $resultdatax->transactionStatus == 'Completed') {
            ...
            if(session('client_id')){
               $cart_info=DB::table('grocery_temp_cart')->where('client_id',session('client_id'))->get(); 
                ...
                DB::table('grocery_temp_cart')->where('guest_id',session('guest_id'))->update(['client_id' => $client_id]);
            ...
            {
                $data['discount']=0;
                $data['total_amount']=$total_price_to_pay;
            }
            ...
            $request->session()->put('guest_order_id', $data['order_id']);
            $data1=array();
